...
#            test: compiles test targets, runs unit and integration tests
# coverage_report: generates gcov/gcovr coverage reports (run after test)
...
coverage_report:	LDFLAGS = $(DEBUG_LDFLAGS)
coverage_report:
...
# Validate doc/Project.xml against tools/project.xsd and run the
# semantic cross-reference checks in tools/lint_project.py.
validate-xml:
	python3 $(PROJECT_ROOT)/tools/lint_project.py
...
help:
	...
	@echo "  validate-xml      - Validate doc/Project.xml (XSD + semantic lint)"
...
.PHONY: all clean install uninstall run test debug release integration_tests build complexity sloc analyze prereqs help coverage_report validate-xml
